...
import com.rxmedical.api.util.DateUtil;
...
public class AnalyzeService {
    ...
    public Map<String, Integer> getLaborScore() {
        ...
        List<Record> finishRecords = saleServiceClient.getFinishRecordsAfter(monthAgoDate).getData();
    ...
    public List<CallAdviceDto> getCallMaterialDiagram(Integer productId) {
        Product product = productServiceClient.findProductById(productId).getData();
        if (product == null) {
            return null;
        }
        ...
        // 取得該商品八週內所有紀錄
        List<History> historyList = saleServiceClient.findProductHistoryAfter(
                                            new GetProductHistoryDto(product, eightWeeksAgo)
                                    ).getData();
